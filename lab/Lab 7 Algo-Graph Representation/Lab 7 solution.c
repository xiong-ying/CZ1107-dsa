...
void adjM2adjL(Graph *g)
{
    ...
    if(g->type != ADJ_MATRIX) {printf("Error\n"); return;}
    ...
    // STEP 3: update union member from adjM to adjL
    g->adj.list = list;
    g->type = ADJ_LIST;
...
void adjL2adjM(Graph *g){
    ...
    // check if the input is adjL
    if(g->type != ADJ_LIST) {printf("Error\n"); return;}
    ...
    // STEP 4: update union member from adjL to adjM
    g->adj.matrix = mat;
    g->type = ADJ_MATRIX;
...
void calDegreeV(Graph g, int *degreeV)
{
    ...
    if(g.type == ADJ_LIST) {
        ...
        for (i=0; i<g.V; i++){
            degreeV[i] = 0;
            for (j=0; j<g.V; j++){ degreeV[i] += g.adj.matrix[i][j];}
        }
    }
}
