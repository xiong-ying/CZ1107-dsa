...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
    ...
    if (ListSize>1){
        ...
        if (ListSize%2 == 0){
        FrontListSize = ListSize/2;
        }
        else{
            FrontListSize = ListSize/2 + 1;
        }
        /*printf("Listsize is %i, front list size is %i.\n", ListSize, FrontListSize);*/

        resultBackList->head = findNode(ll,FrontListSize);

        resultFrontList->head = ll->head;
        tmp = findNode(ll, (FrontListSize-1));
        tmp->next = NULL;
    }
    else if (ListSize==1){
        resultFrontList->head = ll->head;
