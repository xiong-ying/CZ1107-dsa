...
void sumToC(LinkedList* ll, int C, ArrayList* al)
{
    ...
    ArrayNode *tmpA = al->head;


    while (1){
        if (ll->sum+add < C){

            insertNode(ll, indexLL, add);
            add ++;
            indexLL ++;
            //add to ll, add to sum, add++, indexLL++
        }
        else if (ll->sum+add > C){

            indexLL --;
            add = findNode(*ll, indexLL)->item + 1;
            removeNode(ll, indexLL);

        }
        else if (ll->sum+add == C){
            insertNode(ll, indexLL, add);

            tmpA->sizeArray = indexLL+1;
            //printf("SizeArray = %d.\n", tmpA->sizeArray);
            tmpA->itemArray = malloc((indexLL+1)* sizeof(int));
            for(i=0; i<indexLL+1; i++)
            {
                tmpA->itemArray[i] = findNode(*ll, i)->item;
            }

            tmpA->next = malloc(sizeof(ArrayNode));
            tmpA = tmpA->next;
            al->size ++;
            //add to ll, ll to al, add++
