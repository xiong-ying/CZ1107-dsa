...
int main()
{
	...
	int i,j,k;
	int StdInput[Prj+Mtr+2];
	int endIndex = Std+Prj+Mtr+1;

	// Graph vertex is in sequence, by the order of start point 0, students, projects, mentors, end point
    ...
    for(i=0; i<Std; i++){
        ...
        scanf("%i", &StdInput[0]);
        scanf("%i", &StdInput[1]);
        // Get preferred projects, directed from prj to std, prj = +Std
        for (j=0; j<StdInput[0]; j++){
            scanf("%i", &StdInput[j+2]);
            insertAdjVertex(&g.list[StdInput[j+2]+Std], i+1);
            g.E++;
        ...
        for( k=0; k<StdInput[1]; k++){
            scanf("%i", &StdInput[k+StdInput[0]+2]);
            insertAdjVertex(&g.list[i+1], StdInput[k+StdInput[0]+2]+Std+Prj);
            g.E++;
        }
    }
    */
